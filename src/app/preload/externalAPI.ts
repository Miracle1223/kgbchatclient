...
// Incoming call popup (KGBChat custom): the calls plugin announces ringing
// via window.postMessage; relay to the main process for the native popup.
window.addEventListener('message', ({origin, data}) => {
    if (origin !== window.location.origin) {
        return;
    }
    if (data?.type === CALLS_INCOMING_CALL) {
        ipcRenderer.send(CALLS_INCOMING_CALL, data.message);
    } else if (data?.type === CALLS_INCOMING_CALL_ENDED) {
        ipcRenderer.send(CALLS_INCOMING_CALL_ENDED, data.message);
    }
});

// Relay popup join/dismiss back into the calls plugin webapp.
ipcRenderer.on(CALLS_INCOMING_CALL_ACTION, (_: IpcRendererEvent, msg: {action: string; channelID: string; callID: string}) => {
    window.postMessage({type: CALLS_INCOMING_CALL_ACTION, ...msg}, window.location.origin);
});
